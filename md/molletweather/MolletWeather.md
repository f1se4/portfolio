...
'DATA_LECUTRA' is a date which will help us to simplify the model by days, and help us to eliminate some hours non-measured data, but in the future we will try to do hourly as for raining reasons microstate of the weather could be really important.
...
```python
(
    ggplot(df_result,aes(x='Temperatura',y='Humitat relativa',color='Precipitació'))+
    geom_point()+
    orange_dark_theme
).draw();
```
...
```python
(
    ggplot(df_result,aes(x='Pressió atmosfèrica',y='Humitat relativa',color='Precipitació'))+
    geom_point() +
    orange_dark_theme
).draw();
```
    
![png](/static/notebooks/molletweather/output_31_1.png)
...
![png](/static/notebooks/molletweather/output_39_1.png)
